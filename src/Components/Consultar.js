...
function Consultar() {
  ...
  const detalhesFiltrados = useMemo(() => {
    ...
    return detalhesArray.filter(
      (d) => (d?.modelo || 'SEM_MODELO') === modeloSelecionado
    );
  ...
  const buscarProduto = useCallback(async (clusterData) => {
    ...
    try {
      ...
      const url = `${config.API_BASE_URL}/api/clusters/${encodeURIComponent(
        idLimpo
      )}`;
      ...
      const resultadoFormatado = {
        ...dados,
        id: dados.id || idLimpo,
        nome: dados.nome || dados.cluster_nome || `Cluster ${idLimpo}`,
        descricao: dados.descricao || '',
        total_imeis:
          dados.total_imeis ||
          (dados.detalhes_imeis ? Object.keys(dados.detalhes_imeis).length : 0),
      ...
      setErroCamera(`Erro ao buscar informações: ${error.message}`);
      throw error;
    } finally {
      setCarregando(false);
      setMensagemStatus('');
  ...
  const iniciarCamera = useCallback(async () => {
    ...
    try {
      ...
      await qrCode.start({ facingMode: 'environment' }, qrConfig, onQRCodeSuccess);
      ...
      <div className="consultar-header">
        <div className="consultar-header__title">
          ...
          <div className="consultar-subtitle">
            Use a câmera para escanear o QR Code / código de barras.
          </div>
        </div>
      </div>
                  ...
                  <div className="consultar-tableHeader">
                    ...
                    <div className="consultar-tableTools">
                      <select
                        ...
                        onChange={(e) => setModeloSelecionado(e.target.value || null)}
                      >
